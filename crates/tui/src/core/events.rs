...
pub enum Event {
    ...
    /// Live text output streamed from a running sub-agent step. Emitted once
    /// per step when the sub-agent's API response contains a Text block, so
    /// the parent TUI can show intermediate results without waiting for the
    /// sub-agent to finish. Displayed alongside AgentProgress in the history.
    SubAgentStreamDelta {
        /// ID of the sub-agent (e.g. `"agent_abc12345"`).
        agent_id: String,
        /// Current step index within the sub-agent run.
        step: u32,
        /// Text content of the sub-agent's response at this step.
        content: String,
    },
