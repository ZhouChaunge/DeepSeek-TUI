...
pub(super) fn sanitize_thinking_mode_messages(
    ...
    // Truncate reasoning_content on older turns to prevent O(N) token growth.
    // Keep only the most recent DEEPSEEK_REASONING_REPLAY_TURNS turns (default: 3).
    // Older turns are replaced with a placeholder so the API stays happy but the
    // re-sent token budget is bounded regardless of session length.
    let max_replay_turns: usize = std::env::var("DEEPSEEK_REASONING_REPLAY_TURNS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(3);
    // Collect indices of assistant messages that carry real (non-placeholder) reasoning.
    let real_reasoning_indices: Vec<usize> = messages
        .iter()
        .enumerate()
        .filter(|(_, msg)| {
            msg.get("role").and_then(Value::as_str) == Some("assistant")
                && matches!(
                    ...
                    Some(s) if !s.trim().is_empty() && s != "(reasoning omitted)"
                )
        })
        .map(|(i, _)| i)
        .collect();
    if real_reasoning_indices.len() > max_replay_turns {
        let truncate_count = real_reasoning_indices.len() - max_replay_turns;
        for &idx in &real_reasoning_indices[..truncate_count] {
            messages[idx]["reasoning_content"] = json!("(reasoning omitted)");
        }
        logging::info(format!(
            "Reasoning-content replay: truncated {truncate_count} older turn(s) to placeholder \
             (keeping last {max_replay_turns}; set DEEPSEEK_REASONING_REPLAY_TURNS to adjust)",
        ));
    }
