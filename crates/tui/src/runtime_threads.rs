...
impl RuntimeThreadManager {
    ...
    async fn ensure_engine_loaded(&self, thread: &ThreadRecord) -> Result<EngineHandle> {
        ...
        // Compaction enabled so long-running runtime threads auto-compact when the
        // context exceeds the model's threshold. The cycle architecture (issue #124)
        // handles explicit resets; the auto-compactor catches sessions that simply
        // grow without a manual /compact.
