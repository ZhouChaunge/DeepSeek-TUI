        ...
        for block in &response.content {
            match block {
                ContentBlock::Text { text, .. } if !text.trim().is_empty() => {
                    // Relay the text response to the parent TUI so it shows
                    // intermediate sub-agent output before the agent finishes.
                    if let Some(ref tx) = runtime.event_tx {
                        let _ = tx.try_send(Event::SubAgentStreamDelta {
                            agent_id: agent_id.clone(),
                            step: steps,
                            content: text.clone(),
                        });
