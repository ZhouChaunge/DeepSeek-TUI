...
async fn execute_foreground_via_background(
    ...
    // Emit ToolCallProgress at most once per second to keep the TUI status bar fresh.
    ...
    loop {
        ...
        // Emit ToolCallProgress with recent output ~once per second.
        if last_progress_at.elapsed().as_secs() >= 1 {
            if let Some(ref tx) = context.event_tx {
                let output = if !snapshot.stdout.is_empty() {
                    snapshot.stdout.lines().rev().take(3).collect::<Vec<_>>().into_iter().rev().collect::<Vec<_>>().join("\n")
                } else if !snapshot.stderr.is_empty() {
                    snapshot.stderr.lines().rev().take(3).collect::<Vec<_>>().into_iter().rev().collect::<Vec<_>>().join("\n")
                } else {
                    format!("running for {}ms…", snapshot.duration_ms)
                };
                let _ = tx.try_send(crate::core::events::Event::ToolCallProgress {
                    id: task_id.clone(),
                    output,
                });
            }
