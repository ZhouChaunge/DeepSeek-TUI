...
pub struct ToolContext {
    ...
    /// Engine event sender. When set, long-running tools (e.g. foreground
    /// shell) can emit `ToolCallProgress` events so the TUI status bar
    /// shows live output without waiting for tool completion.
    pub event_tx: Option<tokio::sync::mpsc::Sender<crate::core::events::Event>>,
}
...
impl ToolContext {
    ...
    /// Attach the engine event sender so long-running tools can emit progress.
    #[must_use]
    pub fn with_event_tx(mut self, tx: tokio::sync::mpsc::Sender<crate::core::events::Event>) -> Self {
