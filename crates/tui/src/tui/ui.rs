            ...
            while let Ok(event) = rx.try_recv() {
                ...
                match event {
                    ...
                    EngineEvent::MessageComplete { .. } => {
                        ...
                        // RC3: if ThinkingComplete was delayed (event-burst race), last_reasoning
                        // may still be None here. Inline-drain any still-active thinking entry so
                        // reasoning_content is never missing from the persisted assistant message.
                        if app.last_reasoning.is_none() && app.streaming_thinking_active_entry.is_some() {
                            let duration = app.thinking_started_at.take().map(|t| t.elapsed().as_secs_f32());
                            let remaining = app.streaming_state.finalize_block_text(0);
                    ...
                    EngineEvent::ThinkingStarted { .. } => {
                        ...
                        //
                        // RC2: if a previous thinking entry is still active (interleaved
                        // thinking in a multi-block turn), finalize it first so its tail
                        // tokens are not discarded by the unconditional reset() below.
                    ...
                    EngineEvent::SubAgentStreamDelta {
                        agent_id,
                        step,
                        content,
                    } => {
                        // Show a live preview of the sub-agent's text response in
                        // the status bar and agent-progress map so users can follow
                        // along without waiting for the sub-agent to finish.
                        ...
                        let status = format!("step {step} response: {preview}");
                        app.agent_progress
                            .insert(agent_id.clone(), status.clone());
                        app.status_message =
                            Some(format!("Agent {agent_id}: {status}"));
                    }
                    ...
                    EngineEvent::UserInputRequired { id, request } => {
                        if app.approval_mode == ApprovalMode::Auto {
                            // YOLO mode: auto-select the first option for every question
                            // so the agent loop never blocks waiting for human input.
                            let answers = request
                                .questions
                                .iter()
                                .map(|q| crate::tools::user_input::UserInputAnswer {
                                    id: q.id.clone(),
                                    label: q.options[0].label.clone(),
                                    value: q.options[0].label.clone(),
                                })
                                .collect();
                            let _ = engine_handle
                                .submit_user_input(
                                    id.clone(),
                                    crate::tools::user_input::UserInputResponse { answers },
                                )
                                .await;
                        } else {
...
pub(crate) fn apply_engine_error_to_app(
    ...
    // RC1: finalize any in-flight thinking block before clearing the pointer so
    // the cell transitions from live-spinner to interrupted rather than freezing.
    if app.streaming_thinking_active_entry.is_some() {
        let duration = app.thinking_started_at.take().map(|t| t.elapsed().as_secs_f32());
        let remaining = app.streaming_state.finalize_block_text(0);
        finalize_streaming_thinking_active_entry(app, duration, &remaining);
