...
fn render_thinking(
    ...
    // RC4: show a live line-count indicator while streaming so the user can see
    // truncation is active and more content is being generated.
    if streaming && collapsed && rendered.len() > THINKING_SUMMARY_LINE_LIMIT {
        let extra = rendered.len() - THINKING_SUMMARY_LINE_LIMIT;
        ...
        rendered.push(Line::from(vec![
            ...
            Span::styled(
                format!("... ({extra} more lines streaming)"),
                Style::default().fg(palette::TEXT_MUTED).italic(),
            ),
        ]));
    } else if collapsed && rendered.len() > THINKING_SUMMARY_LINE_LIMIT {
        rendered.truncate(THINKING_SUMMARY_LINE_LIMIT);
        truncated = true;
    }
