...
```bash
# ① 直接下载预编译二进制（无需 Node，无需 Rust）
#   前往：https://github.com/Hmbown/DeepSeek-TUI/releases
#   将 `deepseek` 和 `deepseek-tui` 两个文件放到同一 PATH 目录下。

# ② via Cargo（需要 Rust 1.85+，安装地址：https://rustup.rs）
cargo install deepseek-tui-cli deepseek-tui --locked

# ③ via npm（需要 Node.js 16+）
...
预构建二进制覆盖 **Linux x64/ARM64**、**macOS x64/ARM64**、**Windows x64**。
按以下三种方式选择最适合的安装路径：

### 方式一 — 直接下载预编译二进制（推荐，无需任何运行时）

1. 前往 [Releases 页面](https://github.com/Hmbown/DeepSeek-TUI/releases) 下载适合你平台的两个文件，
   例如 `deepseek-linux-x64` 与 `deepseek-tui-linux-x64`。
2. 去掉平台后缀，重命名为 `deepseek` 和 `deepseek-tui`，放到同一个 `PATH` 目录，
   Linux/macOS 还需 `chmod +x`。
3. 运行 `deepseek` —— 无需 Node.js、无需 Rust、无需 Python。
...
适用于 musl、riscv64、FreeBSD 以及早于 v0.8.8 的 ARM64 发行版。
如未安装 Rust，请先访问 <https://rustup.rs>。
...
### 方式三 — npm（需要 Node.js 16+）

已安装 Node.js 的用户可用一行命令完成安装：
...
> **npm 报错？** npm 包只是一个下载包装器，安装时会自动拉取对应平台的预编译二进制。
> 如遇权限不足、网络超时、架构不支持等报错，改用方式一（直接下载）或方式二（Cargo）即可。

其他平台（musl、riscv64、FreeBSD、交叉编译等）请见 [从源码安装](#从源码安装) 章节或
完整的 [docs/INSTALL.md](docs/INSTALL.md)。
