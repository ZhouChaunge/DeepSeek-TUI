...
```bash
# ① Prebuilt binary — no Node, no Rust required
#   Download from: https://github.com/Hmbown/DeepSeek-TUI/releases
#   Place both `deepseek` and `deepseek-tui` side-by-side on your PATH.

# ② via Cargo (Rust 1.85+ — https://rustup.rs)
cargo install deepseek-tui-cli deepseek-tui --locked

# ③ via npm (Node.js 16+ required)
...
Prebuilt binaries are available for **Linux x64/ARM64**, **macOS x64/ARM64**, and **Windows x64**.
Pick the method that fits your setup:

### Method 1 — Prebuilt binary (recommended, no runtime required)

1. Go to the [Releases page](https://github.com/Hmbown/DeepSeek-TUI/releases) and download the two
   assets for your platform, e.g. `deepseek-linux-x64` + `deepseek-tui-linux-x64`.
2. Rename them to `deepseek` and `deepseek-tui` (drop the platform suffix), put both into the
   **same directory** on your `PATH`, and make them executable (`chmod +x` on Linux/macOS).
3. Run `deepseek` — no Node.js, no Rust, no Python needed.

```bash
# Linux x64 example
curl -Lo deepseek https://github.com/Hmbown/DeepSeek-TUI/releases/latest/download/deepseek-linux-x64
curl -Lo deepseek-tui https://github.com/Hmbown/DeepSeek-TUI/releases/latest/download/deepseek-tui-linux-x64
chmod +x deepseek deepseek-tui
sudo mv deepseek deepseek-tui /usr/local/bin/
deepseek
```

### Method 2 — Cargo (Rust 1.85+)

Works on any Tier-1 target including musl, riscv64, FreeBSD, and ARM64 distros that
pre-date our prebuilt binaries. Install Rust first at <https://rustup.rs> if needed.
...
### Method 3 — npm (Node.js 16+ required)

If you already have Node.js installed, npm is the fastest one-liner:
...
> **npm errors?** The npm package is only a thin installer wrapper — it
> downloads the prebuilt binary for your platform and adds it to your PATH.
> If npm itself errors (permission denied, network timeout, unsupported
> architecture) use Method 1 (direct download) or Method 2 (Cargo) instead.

For everything else — musl, riscv64, FreeBSD, cross-compiling — see [Build from source](#install-from-source)
or the full [docs/INSTALL.md](docs/INSTALL.md) walkthrough.
